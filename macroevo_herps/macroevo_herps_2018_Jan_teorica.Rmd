...
a <- capture.output(edata_whales <- getEventData(whales, events.whales, burnin=0.1))
load("./data/simmap_result.RData")
traits <- read.csv("./data/habitat_vipers_all.csv", sep = ";", header = FALSE)
sm.sum <- describe.simmap(res[[1]], plot = FALSE)
```

# {.center}

<h1>Introdução aos métodos em macroevolução</h1>
...
# Interlúdio Matemático {.subtitle data-notes="Para entender processos de Poisson, a gente precisa ter em mente duas distribuições estatísticas em mente. Cada uma delas tem propriedades importantes que vamos passar rapidamente. A primeira é a distribuição Poisson"}
...
# Distribuição Exponencial {data-background="img/exp_dist.jpg" data-notes="A outra distribuição importante é a distribuição exponencial. Ela é comumente usada para descrever tempos de espera em processos sem memória. Isso porque seu parâmetro descreve algo como a meia vida de um processo, que independe do que acontece no momento anterior."}

> - Variáveis contínuas (tempo de espera nesse caso)
> - Um parâmetro $\lambda$

![](img/exponential_dist.png)

# Processo de Poisson {.subtitle data-notes="Vamos então conversar sobre os dois modelos básicos usados para estimar as taxas de diversificação. O primeiro deles é chamado de Processo de Poisson. O jeito mais simples de descrever um PP é pensando em uma tarde ensolarada numa rodoviária."}

# {data-background="img/rodoviaria.jpg" data-background-size=90% data-notes="Voltando então ao nosso Processo de Poisson. Quem nunca passou uma tarde agradável numa rodoviária contando quantos ônibus chegavam (e/ou saíam) e o tempo entre as chegadas/saídas, não é mesmo?"}
...
![](img/phylo_scheme_simple.png){bg="white"}
...
<br />
<br />
![](img/exponential_dist.png){width="1200px"}
...
<br />
<br />
![](img/wait_a_minute.gif){width="800px"}

# {data-notes="O primeiro método que vamos mostrar é o BAMM. Esse método vem sendo amplamente utilizado desde sua publicação, e consiste em estimar taxas de especiação e extinção usando modelos birth-death variando no tempo, ao mesmo tempo em que testa para diferentes configurações de regimes de diversificação" .center}

<br />
<br />
![](img/bamm_logo.png){width="1200px" bg="#5c6565"}
...
# {.center data-notes="Não sei se vocês repararam, mas vários dos shifts caíram na mesma região, indicando que a probabilidade de ele de fato existir é mais alta do que de outros. Como as cadeias de MCMC do BAMM são bastante longas (da ordem de 50M de iterações), seria impossível olhar para todas as configurações possíveis. Por isso, olhamos para as com maior probabilidade posterior associada."}

```{r echo = FALSE, warning = FALSE, message = FALSE}
css <- credibleShiftSet(edata_whales, expectedNumberOfShifts = 1, threshold = 5, set.limit = 0.95)
b <- capture.output(plot.credibleshiftset(css))
```

# {.center data-notes="Olhando a filogenia a gente consegue ter uma noção do que está acontecendo, porém existem outras formas de analisar os dados. Uma das formas mais comuns é o Rate Through Time Plot. As análises possíveis com o BAMM são muitas, então não dá tempo de falar sobr tudo. Vamos agora mostrar um exemplo prático da aplicação desse modelo."}

```{r echo = FALSE, warning = FALSE}
plotRateThroughTime(edata_whales, ratetype = "speciation")
```  

# {data-background="img/alencar_et_al_2016.png" data-background-size=80% data-notes="Ok, conseguimos identificar e pensar em hipóteses sobre qual(is) fatores podem estar relacionados a mudanças nas taxas, mas existe uma classe de modelos específica para avaliarmos a influência de traits nas taxas de diversificação. Essa classe de modelos é conhecida como xxSSE, e estão implementados no famoso pacote diversitree"}

# 

<br />
![](img/diversitree_logo.png){width="800px" bg="#5c6565"}

# BiSSE

**Bi**nary **S**tate **S**peciation and **E**xtinction

![](img/bisse_scheme.png){width="1400px"}
...
<br />
<br />
![](img/bisse_eq_sp.png){width="1200px"}

# BiSSE {data-notes="Além disso, precisamos levar em consideração os dois estados possíveis. Um raciocínio idêntico pode ser aplicado à extinção, então terminamos com um conjunto de 2 "}

<br />
<br />
![](img/bisse_eq_diff.png){width="1200px"}
...
# Aplicações {.subtitle data-notes="Vou então agora ver se vocês fizeram a lição de casa. No artigo que acabamos de publicar na ProcB, qual foi o modelo que usamos para avaliar se havia diferenças entre as taxas de acordo com cada um dos diferentes habitats das víboras?"}

# Aplicações

<br />
![](img/musse_vipers.png){width="900px"}

# Aplicações {data-notes="Vou então agora ver se vocês fizeram a lição de casa. No artigo que acabamos de publicar na ProcB, qual foi o modelo que usamos para avaliar se havia diferenças entre as taxas de acordo com cada um dos diferentes habitats das víboras? Através das taxas do MuSSE, não foi possível observar diferenças nas taxas dependendo do microhabitat, ao contrário do que imaginávamos. Existem diferentes razões (biológicas ou metodológicas) que podem justificar esses resultados, mas não entraremos em detalhes. Para isso, leiam o artigo :D"}

![](img/musse_vipers_trans.png){width="900px"}

# Evolução fenotípica {.subtitle data-notes="Apesar de métodos como os xxSSE nos permitirem estimar as taxas associadas a cada estado de um caráter, a integração sobre todas as possibilidades de história e estados ancestrais desse caráter envolvidos nas estimativas nos impede de estimar a história de transição/evolução do caráter em questão. Para isso, precisamos lançar mão de métodos que analisam explicitamente a evolução dos caracteres independentemente dos processos de diversificação."}

# Cadeias de Markov {data-notes="Um segundo conceito importante para entendermos os métodos é o conceito de Cadeias de Markov. Provavelmente vocês já ouviram falar esse nome quando pensam em métodos filogenéticos bayesianos - MCMC significa Monte Carlo Markov Chain - mas em termos simples, uma cadeia de Markov é um processo onde os estados ao longo do tempo só dependem de duas coisas: do estado no instante logo anterior e das taxas de transição para diferentes estados. Esse conceito será importante tanto para os métodos de diversificação quanto de evolução morfológica."}

![](img/markov_chain.png){height="550px" bg="white"}

# Cadeias de Markov {data-notes="Mas o que são cadeias de Markov? Vamos pensar nume exemplo simples como a condição do tempo em um determinado lugar. Podemos pensar em dois estados diferentes: dia bonito e dia feio. Lembrando da propriedade de ser sem memória, uma cadeia de Markov para estimar as probabilidades de cada tipo de dia nesse caso vai levar em consideração apenas as condições do dia anterior. São processos sem memória, onde precisamos somente de duas informações para tentar estimar o estado de um caráter no instante t+1: o estado no instante t e as probabilidades de transição entre estes estados. Pensando num caráter binário facilita as coisas, já que são somente 2 possibilidades para cada estado que devem somar 1 (já que se tratam de probabilidades)."}

<br />
![](img/markov_chain_example.png){width="1200px"}

# {data-background="img/cubo_2x2.jpg" data-background-size=70% data-notes="Nosso objetivo com esses métodos é estimar essas taxas de transição. Podemos encarar a estimativa dessas taxas como um cubo mágico. Para chegar na melhor configuração você precisa ir alterando as cores em cada célula em diferentes combinações. Essa é uma aproximação do processo de otimização para estimativa dos parâmetros."}

# {data-background="img/cubo_20x20.jpg" data-background-size=65% data-notes="Porém, quando começamos a acrescentar estados, as coisas podem facilmente se tornar um pesadelo."}

# Stochastic Mapping {.center data-notes="Existem muitos métodos para evolução fenotípica, então hoje vamos conversar somente sobre os dois métodos usados no nosso artigo. Estes dois métodos são utilizados em conjunto (o segundo depende do primeiro). O primeiro que vamos er consiste basicamente em reconstruir a história evolutiva do caráter, gerando diferentes cenários que podem ter levado à configuração de estados observada hoje. À esse método damos o nome de stochastic mapping."}

# {data-notes="Vamos então usar o exemplo dos viperídeos. Essa aqui é a árvore completa que a Laura gerou, com o caráter Microhabitat de cada espécie plotado nos terminais. O Stochastic Mapping consiste em estimar os valores das taxas de transição entre os microhabitats (através de um modelo Mk) e reconstruir histórias evolutivas possíveis que geraram a configuração que a gente vê hoje em dia. Como já vimos no exemplo do diversitree, não temos nenhuma certeza sobre a história evolutiva do grupo (filogenias são hipóteses afinal), então o ideal é incorporar as incertezas nas análises gerando inúmeros mapeamentos."}
...
# {data-notes="Vou mostrar 10 mapeamentos que fiz, e podemos notar que apesar de sutis existem importantes diferenças entre cada um deles"}

```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[1]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[2]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[3]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[4]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[5]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[6]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[7]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[8]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[9]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
```{r echo = FALSE, warning = FALSE, verbose = FALSE, out.width = 1080}
par(mar=c(1,0,0,1), bg = "black")
plotSimmap(res[[1]][[10]], colors = setNames(brewer.pal(4, "Set3"), 1:4), type = "fan", fsize = 0, lwd = 5)
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```
...
# {data-notes="De posse de um número grande de mapeamentos para uma mesma topologia, podemos calcular estatísticas de resumo para facilitar a compreensão dos dados. Um exemplo é esse onde calculamos as probabilidades de cada nó possuir cada um dos estados"}
...
tiplabels(pch = 21, col = "black", adj = 1, bg = brewer.pal(4, "Set3")[traits[match(res[[1]][[1]]$tip.label, traits[,1]), 2]], cex = 1.3)
nodelabels(node = 1:res[[1]][[1]]$Nnode + Ntip(res[[1]][[1]]), pie = sm.sum$ace, cex = 0.6, piecol = brewer.pal(4, "Set3"))
legend("bottomright", legend = c("Open + Closed", "Open", "Closed", "Arboreal"), pch = 19, col = brewer.pal(4, "Set3"), text.col = "white")
```

# Modelos de Evolução de Caracteres Contínuos {.subtitle data-notes="O Stochastic Mapping é comumente usado para reconstrução de caracteres discretos. No entanto, caracteres de interesse (como body size por exemplo) são contínuos. Como pensar em taxas de transição entre caracteres contínuos é meoi estranho, existe outra forma de lidar com eles. Esse outro método está associado ao simmap (pois depende dele) e consiste em ajustar diferentes modelos de evolução de caracteres contínuos baseados na história evolutiva de um caráter discreto. Esses métodos são chamados de OUwie. Para explicar como eles funcionam, primeiro vamos conhecer dois populares modelos de evolução de caracteres contínuos."}
...
# {data-background="img/results_ouwie.png" data-background-size=85%}

# Recapitulação {.subtitle}
