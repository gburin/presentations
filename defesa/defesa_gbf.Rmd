...
# {data-notes="Além disso, o planeta passou por fases de aumento e diminuição do número de espécies, como por exemplo as conhecidas 5 (ou 6) extinções em massa" data-background="img/mass_extinctions.jpg" data-background-size=90%}

# Macroevolução {.center}

# {data-notes="Macroevolução é a área que estuda a evolução acima do nível da espécie. Mudança no número e/ou forma das espécies em escalas de tempo bastante longas. Focarei só na diversificação. O que procuramos entender é como acontece e o que afeta a formação e o desaparecimento de novas espécies, através dos processos de especiação e extinção." data-background="img/deeptime_evolution.png" data-background-size=60%}
...
# {data-notes="Esses processos acontecem ao longo de milhares ou milhões de anos. Basicamente o balanço entre esses dois processos, modulados por diferentes mecanismos como por exemplo eventos geológicos, que determina a diversidade em um momento no tempo." data-background-video="img/marine_fossil_history.mp4" data-background-size=75%}

# {data-notes="Como podemos estudar isso então? Precisamos de uma máquina do tempo" data-background="img/time_machine.png"}
...
# {data-notes="Com isso, foi por exemplo possível construir a árvore da vida, que contém quase toda a informação molecular disponível hoje e ajudou a elucidar um pouco melhor a relação entre os grandes grupos" data-background="img/treeoflife.jpg" data-background-size=50%}

# {data-notes="Para dar um exemplo, no trabalho da Laura eles construíram uma filogenia para Viperidae, e observaram uma mudança nos padrões de diversificação para o grupo neotropical. Uma exigência em estudos de macroevolução (bem como todo estudo decente) é a quantidade de dados" data-background="img/alencar_et_al_2016.png" data-background-size=80%}

# {data-notes="Uma das razões é que é muito legal falar que vc trabalha com os únicos dinossauros vivos" data-background="img/dinosaur_blood.jpg" data-background-size=45% .center}

<h1 style="background:rgba(0,0,0,0.75);"> Por que aves?</h1>

# {data-notes="Aves são parentes próximos dos grupos de dinossauros extintos" data-background="img/theropoda_phylo.png" data-background-size=45%}

# {data-notes="É um grupo com uma quantidade imensa de dados" data-background="img/historical_data.png" data-background-size=50%}

# {data-notes="É um grupo com uma quantidade imensa de dados" data-background="img/markmybird.png" data-background-size=100% .center}
...
# {data-notes="É possível ver por exemplo que as migrações estão mudando tanto no tempo quanto no espaço. Hurlbert & Liang 2012 PLoS ONE" data-background="img/citizen_science_change_phenology.png" data-background-size=55%}

# {data-notes="Ocupam todos os continentes. Estamos acostumados com o estudo da evolução de uma ou poucas linhagens ao longo de algumas gerações. É portanto um grupo de grande interesse para a macroevolução" data-background="img/kissling_et_al_2012.png" data-background-size=100%}
...
# {data-notes="No entanto, é um grupo para o qual dados moleculares são abundantes e permitiram estabelecer razoavelmente bem suas relações de parentesco" data-background="img/jetz2012.png" data-background-size=70%}

# {data-notes="As aves se originaram a cerca de 120MY, então não é de se estranhar que sejam um grupo extremamente rico em morfologias" data-background="img/Bird_Diversity2.png" data-background-size=90%}
...
# {data-notes="As diferenças nas ecologias também se refletem nos diferentes serviços ecoloógicos que as aves desempenham, como dispersão de sementes, predação, consumo de carcaças, e polinização. Nesses serviços (que muitas vezes tem relação com a ecologia trófica das espécies) as espécies de ave interagem com diversas outras espécies de diferentes grupos, podendo essas interações serem antagonísticas ou mutualistas" data-background="img/bird_services.png" data-background-size=77%}
...
# Como? {data-notes="Usando um pacote que contém modelos que estimam as taxas de diversificação associadas a diferentes estados de um caráter" data-background="img/r_diversitree.png" data-background-size=70%}

<img class="fragment" src="img/bisse_example.png" width="600px"/>

# Como? {data-notes="Taxas de especiação, extinção e transição pra cada estado. Transições são mudanças anagenéticas, ou seja, sem especiação"}
...
# {data-notes="Resultados para net diversification" data-background="img/result1_omniv.png" data-background-size=90%}

# {data-notes="Essa net diversification menor para onívoros vem vem de terem menor especiação e/ou maior extinção que as demais guildas." data-background="img/cap1fig2_black.png" data-background-size=90%}

# {data-notes="Essa net diversification menor para onívoros vem vem de terem menor especiação e/ou maior extinção que as demais guildas." data-background="img/cap1fig2_black_highlight.png" data-background-size=90% data-transition="none"}
...
<img class="fragment" style="background:black; border:0px;" src="img/competition.png" width=65%/>
...
# {data-notes="Como comentei anteriormente, existem duas formas de estimar as taxas, e diferentes grupos ocupam diferentes posições no espectro da disponibilidade de dados fósseis e moleculares" data-background="img/data_types.png" data-background-size=60%}
...
# {data-notes="Porém, existe o problema de se detectar extinção através de filogenias moleculares" data-background="img/phylo_ltt.png" data-background-size=60%}

# {data-notes="Grupos viventes podem estar em expansão, saturação ou declínio, e normalmente o último caso imagina-se ser comum e é pouco estudado" data-background="img/growth_scenarios.png" data-background-size=100%}
...
knitr::include_graphics('./img/sim_scenarios.png')
```
...
> - 214 filogenias
> - 4 grandes grupos de vertebrados
...
> - Baixas taxas de falso positivo
...
# {data-notes="No terceiro capítulo testamos a relação entre taxas de diversificação e o papel ecológico desempenhado pelas espécies de aves em redes de frugivoria" data-background="img/cap3fig1.png" data-background-size=80% .center}
...
<h2 style="background:rgba(0,0,0,0.75);">Espécies mais centrais pertencem a linhagens mais estáveis/confiáveis</h2>

<h2 style="background:rgba(0,0,0,0.75);">Interação positiva com prec: "tropicalização"</h2>

# {data-notes="Disponibilidade de água pode estar relacionado com a fenologia. Diferentes espécies tem relações diferentes com o clima, mas essa relação é geralmente mantida ano após ano." data-background="img/tree_phenology.png" data-background-size=22%}

<img class="fragment" src="img/tree_phenology_climate.png"/>

# Discussão {data-notes="Tradeoff: redes quantitativas mostraram mais relações do que redes binárias" .center}

> - Relação negativa: espécies mais centrais vem de linhagens mais estáveis/"confiáveis" (*species sorting*)
> - Disponibilidade de água (total e sazonal): Fenologia
